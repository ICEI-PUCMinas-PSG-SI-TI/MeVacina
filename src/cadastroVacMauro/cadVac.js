...
async function cadNotificacao(notif){

    const notificacao = {
        idPaciente: notif.idPaciente,
        title: notif.title,
      ...
      try {
        ...
        if (response.ok) {
            ...
            alert("Erro ao cadastrar a notificação.");
        }
    } catch (error) {
        console.error("Erro ao conectar ao servidor:", error);
        alert("Erro ao conectar ao servidor.");
  ...
  async function mostrarVacinas() {
      ...
      resultado.innerHTML = vacinas.map(vacina => `
          <p>Vacina: ${vacina.vacina} - Grupo: ${vacina.grupo || "Não especificado"} - Doses restantes: ${vacina.dose} ${vacina.dosesRestantes}</p>
      `).join("");
  }
